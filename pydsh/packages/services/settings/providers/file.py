...
用户文档位置：``~/.pydsh/settings.json`` 的顶层（复用现有 settings.json 形态），
...
from pydsh.infrastructure.config.files import user_settings_path, load_json, save_json
...
class FileSettingsService(SettingsService, CapabilityProvider):
    ...
    def _commit(self, scope: SettingsScope, next_section: dict) -> None:
        ...
        try:
            ...
            print(f"[pydsh] settings: 写入 namespace {ns!r} 失败，保留旧值：{exc}")
            return
        for watcher in list(scope._watchers):
            try:
                watcher(scope.get(), prev)
            except Exception as exc:
                print(f"[pydsh] settings: watcher 异常：{exc}")
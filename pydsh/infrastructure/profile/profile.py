...
覆盖链（后覆盖前）：默认 [pydsh.base] < 命名 profile < 项目 < 用户 < argv。
``--profile``：文件存在 → 当路径（argv 覆盖）；否则 → 当命名 profile 名（~/.pydsh/profiles/<n>.yaml）。
"""
...
def _expand_bundles(bundle_names: list[str]) -> list[str]:
    ...
    返回拓扑序列表（依赖在前，base 排最前）。短名（如 base/tui）自动归一为 pydsh.xxx。
    """
    seen: set[str] = set()
    result: list[str] = []

    def _normalize(name: str) -> str:
        return name if name.startswith("pydsh.") else f"pydsh.{name}"
...
def resolve_profile(
    ...
    覆盖链（全部显式指定，无隐式加载）：
      1. 默认 bundles=[pydsh.base]
      2. 命名 profile（若 profile 给的是「名字」→ ~/.pydsh/profiles/<name>.yaml）
      3. argv（--profile 给「路径」时，作为最高覆盖层）

    - bundles 覆盖：取「最后写 bundles 的那层」的非 base 部分 + [pydsh.base]。
    ...
    for bname in expanded:
        ...
        if bundle is not None:
            ...
            print(f"[pydsh] 警告：未知 bundle {bname!r}，跳过", file=__import__("sys").stderr)

...
发现约定：plugins 名单中 ``name`` 以 ``pydsh.app-`` 开头者为 app 插件，取第一个。
...
class AppPlugin:
    ...
    def assert_valid(plugin: Any) -> None:
        """校验一个 app 插件：name 以 ``pydsh.app-`` 开头，inject 含 ``agent_loop``。"""
...
def find_app_plugin(ctx: Any, entries: list) -> Callable | None:
    ...
    ``pydsh.app-`` 前缀且已在 ctx 装配激活的插件。无 app 插件返回 ``None``。

...
- profile 覆盖链（默认 [pydsh.base] < 命名 < 项目 < 用户 < argv）得到最终 plugins 名单；
- 内置 base + 第三方插件同一发现机制：entry-point 组 `pydsh.plugins`（条目名 = 插件名）；
- config/root 是运行时值，经 `pydsh.config` / `pydsh.root` 两个插件的 ``SET`` 槽注入。
...
from pydsh.packages.services.workspace.providers import root as root_plugin
...
def load_project(
    ...
    *,
    ...
    - ``profile``：profile 名（或路径）；None = 默认 [pydsh.base]。
...
_PERSISTENCE_PROVIDERS = {
    "jsonl": "pydsh.persistence-jsonl",
    "sqlite": "pydsh.persistence-sqlite",
}
...
def _profile_plugins(
    ...
    return [
        r for r in merged
        if not (quiet and r.name == "pydsh.trace-render")
    ]
...
- ``pydsh [--profile <name>] [--patch <file>...] [--storage jsonl|sqlite] [--session <id>] [dir] [app-args...]``
  拣选 app 插件（plugins 名单里 ``pydsh.app-`` 前缀者）并调用其 ``apply(ctx, args)``。
- ``pydsh replay <path> [--session-id ID]``  从 jsonl 重放会话时间线（纯数据读取，不经 app 插件）。
- ``pydsh plugin add|remove|ls ...``          安装/卸载/列举插件。

``--profile``：文件存在 → 当 argv 覆盖路径；否则 → 当命名 profile 名（~/.pydsh/profiles/<n>.yaml）。
"""
...
from pydsh.packages.services.session.reporting import replay_session, load_session_events
    ...
    while i < len(argv):
        ...
        if tok == "--profile":
            ...
            if args.storage not in _STORAGE_CHOICES:
                print(
                    f"[pydsh] 非法存储后端 {args.storage!r}（可选 {sorted(_STORAGE_CHOICES)}）",
                    file=sys.stderr,
                )
                raise SystemExit(2)
            i += 2
...
def _boot(args: BootArgs) -> int:
    ...
    # 若 profile 名不存在于 ~/.pydsh/profiles/，则视为额外 bundle 名追加到 base
    extra_bundles: list[str] = []
    if profile_name is not None:
        from ..profile import profile_path as _pp

        if not _pp(profile_name).is_file():
            # 归一为 bundle 名（bundle 文件命名 pydsh.<name>.yaml）
            bundle_name = profile_name if profile_name.startswith("pydsh.") else f"pydsh.{profile_name}"
    ...
    # 拣选 app 插件：plugins 名单里第一个 pydsh.app- 前缀者
    ...
    if apply_fn is None:
        print(
            "[pydsh] 无 app 插件：请在 profile 的 plugins 里激活一个 pydsh.app-* 前端"
            "（例如 --profile tui）",
            file=sys.stderr,
        )
        return 1
...
def _cmd_replay(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="pydsh replay", description="重放一个会话的时间线")
...
def _cmd_plugin(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="pydsh plugin", description="安装/卸载/列举插件")

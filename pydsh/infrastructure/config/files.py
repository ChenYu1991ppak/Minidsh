...
用户级在 ``~/.pydsh/``（或 ``$PYDSH_HOME``），项目级在 ``<project>/.pydsh/``。
...
def user_config_dir() -> Path:
    """用户级配置根目录：``$PYDSH_HOME`` 或 ``~/.pydsh``。"""
    home = os.environ.get("PYDSH_HOME") or Path.home() / ".pydsh"
    return Path(home)
...
def project_dir(project_root: str | Path) -> Path:
    """项目级配置目录：``<project>/.pydsh/``。"""
    return Path(project_root) / ".pydsh"

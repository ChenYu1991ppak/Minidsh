...
export interface AcpClientOptions {
  /** Path to pydsh binary (default: "pydsh") */
  bin?: string;
...
export class AcpClient extends EventEmitter {
  ...
  async start(overrides?: Partial<AcpClientOptions>): Promise<void> {
    ...
    // PYDSH_BIN 由 launcher（pydsh --profile tui）注入；否则默认 "pydsh"
    const bin = process.env.PYDSH_BIN ?? opts.bin ?? "pydsh";
    // ACP 后端 profile：默认 acp，可用 PYDSH_ACP_PROFILE 覆盖
    const profile = process.env.PYDSH_ACP_PROFILE ?? "acp";

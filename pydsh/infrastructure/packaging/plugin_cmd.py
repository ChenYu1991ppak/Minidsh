"""`pydsh plugin` 子命令：add / remove / ls。
...
用户级插件激活写 ``~/.pydsh/profile.yaml`` 的 ``plugins:`` 键。
"""
...
def plugin_add(pkg_spec: str, *, pip: list[str] | None = None) -> int:
    ...
    if result.returncode != 0:
        print(f"[pydsh] pip 安装失败：{result.stderr.strip()}", file=sys.stderr)
        return 1
    ...
    if new_names:
        ...
        for n in new_names:
            print(f"[pydsh] 已激活插件 {n}")
    else:
        print("[pydsh] 未发现新的 entry-point 插件（检查包的 [project.entry-points.\"pydsh.plugins\"]）")
    return 0
...
def plugin_remove(name: str) -> int:
    ...
    if len(remaining) == len(entries):
        print(f"[pydsh] 插件 {name!r} 不在用户 profile 中", file=sys.stderr)
        return 1
    _write_user_plugins(remaining)
    print(f"[pydsh] 已从用户 profile 移除插件 {name}")
    return 0

...
标准化（plugin-def）：本模块只消费 ``Plugin`` 实体，不再直接读原始回调的
``inject`` 或拼 ``name``。归一化在 ``Context.plugin`` 完成（含名字唯一性把关），
fiber 拿到的是已定型的 name/inject/factory。
"""
...
class Fiber:
    ...
    def __init__(self, ctx, plugin: Plugin, config=None):
        self.ctx = ctx
        self.config = config
        self.name = plugin.name
        self.inject = list(plugin.inject)
        self._body = plugin.factory
    ...
    def activate(self):
        ...
        插件体 = Plugin.factory；类形态的 factory 是类本身，调用即实例化（constructor(ctx)）。
        """
